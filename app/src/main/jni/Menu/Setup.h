...
//#include "Includes/curlpp/cURLpp.hpp"
//#include "Includes/curlpp/Options.hpp"
//#include <Includes/curlpp/Easy.hpp>
//#include <Includes/curlpp/Exception.hpp>
...
/*
std::string httpgetrequest(char* link) {
    //curlpp::Cleanup myCleanup;
        ...
        os << (const char*)curlpp::options::Url(link);
        return(std::string(os.str()));
}
*/
std::string checkmenustatus(char* link, char* statuslink, char* versionlink) {
    std::string status = "OK", version;
    try {
        //status = httpgetrequest(statuslink);
        //version = httpgetrequest(versionlink);
        if (VERSION != version) {
            status = "The mod menu is outdated - please download the latest version at "
            + std::string{link};
        }
    }
...
void authorizemenu(JNIEnv *env, jobject thiz, jobject ctx) {
    std::string status = checkmenustatus("https://github.com/HorridModz/Polywar-Mod-Menu", "https://github.com/HorridModz/Polywar-Mod-Menu/main/menustatus.txt", "https://github.com/HorridModz/Polywar-Mod-Menu/main/menuversion.txt");
    sleep(2.5);
    if (status == "[GetStatusFailed]") {
        ...
        std::string basemessage = "Sorry, the menu could not be authorized: ";
        ...
        LOGD("Successfully authorized menu");
        menuenabled = true;
    }
}
...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    ...
    //Dialog Example
    //NOTE TO SELF: THIS MAY CAUSE CRASH
    //https://stackoverflow.com/questions/27087983/unable-to-add-window-token-null-is-not-valid-is-your-activity-running
    //setDialog(ctx,env,OBFUSCATE("Test"),OBFUSCATE("Hi this is dialog test!"));

    //Toast Example
    Toast(env,ctx,"Modded by HorridModz\nDiscord: User123456789#6424",ToastLength::LENGTH_LONG);
    menuauthorized = true; //disables menu authorization
    menuenabled = true; //disables menu authorization
    if (!menuauthorized) {
        authorizemenu(env, thiz, ctx);
    }
    do {
        sleep(1);
    } while (!menuenabled);
